...
define.Class('wysiwyg.viewer.managers.actions.ScreenInAction', function(classDefinition) {
    ...
    def.binds(['_onScrollEvent', '_onResizeEvent', '_onTickEvent', '_onPageTransitionStart', '_onPageTransitionComplete']);
    ...
    def.fields({
        ...
        _pageTargetsToPlay: [],
        /** Default threshold in percents before animating an element when it enters the viewport */
        _defaultInViewportThreshold: 10,
    ...
    def.methods({
        ...
        _addListeners: function() {
            $(window).addEvent('scroll', this._onScrollEvent);
            $(window).addEvent('resize', this._onResizeEvent);
        ...
        _removeListeners: function() {
            $(window).removeEvent('scroll', this._onScrollEvent);
            $(window).removeEvent('resize', this._onResizeEvent);
        ...
        _populateSiteTargetsPlayData: function(targetsIds) {
            ...
            _.forEach(targetsIds, function(targetId) {
                ...
                this._siteTargetsPlayData[targetId].topPlayTrigger = this._getTopPlayTriggerByTargetId(targetId);
            }, this);
        ...
        /**
         * Calculate the top coordinate of page scroll to trigger animation on the target component with the passed id
         * @param {String} targetId
         * @returns {Number}
         * @private
         */
        _getTopPlayTriggerByTargetId: function(targetId) {
            var inViewportThreshold = 100;
            var domElement = this._getDomElementByTargetId(targetId);
            var domElementCoordinates = domElement.getBoundingClientRect();

            var yOffset = this.resources.W.Utils.Layout.isComponentOrParentsFixedPositioned(domElement.$logic) ? 0 : window.pageYOffset;

            // iterate and set min threshold
            _.forEach(this._getAnimationsByTargetId(targetId), function(animation) {
                var behaviorDataThreshold = this._defaultInViewportThreshold;

                if (animation.options) {
                    if (typeof animation.options.screenInThreshold !== 'undefined') {
                        behaviorDataThreshold = parseInt(animation.options.screenInThreshold);
                    }
                }

                inViewportThreshold = Math.min(inViewportThreshold, behaviorDataThreshold);
            }, this);

            return domElementCoordinates.top + yOffset + domElementCoordinates.height * (inViewportThreshold / 100);
        },
        ...
        _animateTargets: function() {
            ...
            while (i--) {
                ...
                var isTargetInViewport = this._isTargetInViewport(targetId);
        ...
        /**
         * Return whether target component with passed id is inside the viewport
         * @param {String} targetId
         * @returns {boolean}
         * @private
         */
        _isTargetInViewport: function(targetId) {

            var targetTop = this._siteTargetsPlayData[targetId].topPlayTrigger;
            return (targetTop >= window.pageYOffset && targetTop <= window.getSize().y + window.pageYOffset);
        },
        ...
        _getAnimationsByTargetId: function(targetId) {
            ...
            _.forEach(behaviors, function(behavior) {
                var animation = this._animations.getAnimationDefinition(behavior.name);
                if(animation){
