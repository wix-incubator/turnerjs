define.Class('wysiwyg.viewer.utils.TransitionUtils', function (classDefinition) {
    /** @type bootstrap.managers.classmanager.ClassDefinition */
    var def = classDefinition;

    def.binds([
        '_swipeHorizontalTransition',
        '_swipeHorizontalTransitionFullScreen',
        '_swipeVerticalTransition',
        '_swipeVerticalTransitionFullScreen',
        '_noTransition',
        '_inOutTransition',
        '_crossFadeTransition',
        '_shrinkFadeTransition',
        '_trackCallback'
    ]);
    ...
    def.methods(/** @lends {wysiwyg.viewer.utils.TransitionUtils.prototype}**/{
        /**
         * @constructs
         */
        initialize: function () {
        ...
        getTransition: function (transitionID) {
            //transitions should not happen in mobile and tablet
            if(this.resources.W.Config.mobileConfig.isMobileOrTablet()){
...
//                case Constants.TransitionTypes.NONE:
        ...
        _inOutTransition: function (currentDiv, incomingDiv, direction, duration, onComplete) {
            ...
            if (this._useCSS3 && transitionProp) {
                ...
                currentDiv.setStyles({"opacity": "1.0"});
                incomingDiv.setStyles({"opacity": "0.0", "visibility": "visible"});
                this._Tween.to(currentDiv, duration, {   opacity: 0.1, ease: "strong_easeOut"   });
                this._Tween.to(incomingDiv, duration,
                    {   opacity: 1.0,
                        ease: "strong_easeIn",
                        onComplete: function () {
