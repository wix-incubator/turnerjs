...
//#WOH-2715
/**
 * @class wysiwyg.deployment.it.editor.ComponentsDriver
 */
define.Class("wysiwyg.deployment.it.editor.ComponentsDriver", function (classDefinition) {
    ...
    def.methods({
        ...
        createComponent: function (command, componentType) {
            var currentComponentCount = this._getComponentCount(componentType);
            this.resources.W.CommandsNew.executeCommand('WEditorCommands.AddComponent', {compType: command});
            return this._waitOnComponentCount(componentType, currentComponentCount);
        },

        createCompFactoryComponent: function (preset) {
            var currentComponentCount = this._getComponentCount(preset.compType);
            this.resources.W.CommandsNew.executeCommand('WEditorCommands.AddComponent', preset);
            return this._waitOnComponentCount(preset.compType, currentComponentCount);
        },
        ...
        _waitOnComponentCount: function (componentType, initialCount) {
            var dfd = Q.defer(),
                that = this,
                wait = function (count) {
                    setTimeout(function () {
