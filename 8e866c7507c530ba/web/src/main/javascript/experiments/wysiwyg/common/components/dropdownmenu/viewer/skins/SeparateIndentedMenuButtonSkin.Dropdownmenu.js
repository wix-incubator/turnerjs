define.experiment.skin('wysiwyg.common.components.dropdownmenu.viewer.skins.SeparateIndentedMenuButtonSkin.Dropdownmenu', function(skinDefinition, experimentStrategy){
    ...
    def.iconParams({'description': 'Separate Indented Buttons Menu', 'iconUrl': '/images/wysiwyg/skinIcons/menu/separateindented.png', 'hidden': false, 'index': 16});
    ...
    def.skinParams([
        ...
        {'id': '100%', 'type': Constants.SkinParamTypes.OTHER, 'defaultValue': 'width:100%; height:100%;', 'noshow': true},
    ...
    def.css([
        ...
        '%.shade% {[rd] background: url([tdr]indented_bg_inverted.png) center top repeat-x, url([tdr]indented_bg_inverted.png) center bottom repeat-x; position: absolute; top: 0; bottom: 0; left: 0; right: 0;}',
        '%menuContainer% > li:hover > %.gap% %.shade% {background: url([tdr]indented_bg_inverted.png) center top repeat-x, url([tdr]indented_bg_inverted.png) center bottom repeat-x;}',
        '%menuContainer% > li.selected > %.gap% %.shade% {background: url([tdr]indented_bg.png) center top repeat-x, url([tdr]indented_bg.png) center bottom repeat-x;}',
        '%menuContainer% > li.selected:hover > %.gap% %.shade% {background: url([tdr]indented_bg.png) center top repeat-x, url([tdr]indented_bg.png) center bottom repeat-x;}',
        ...
        ' .subMenu li:hover %.shade% {background: url([tdr]indented_bg_inverted.png) center top repeat-x, url([tdr]indented_bg_inverted.png) center bottom repeat-x; border-radius: 0;}',
        ' .subMenu li.selected %.shade% {background: url([tdr]indented_bg.png) center top repeat-x, url([tdr]indented_bg.png) center bottom repeat-x; border-radius: 0;}',
        ' .subMenu li.selected:hover %.shade% {background: url([tdr]indented_bg.png) center top repeat-x, url([tdr]indented_bg.png) center bottom repeat-x;}',
