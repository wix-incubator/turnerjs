define.experiment.skin('wysiwyg.common.components.dropdownmenu.viewer.skins.SeparateBasicMenuButtonSkin.Dropdownmenu', function(skinDefinition, experimentStrategy){
    ...
    def.iconParams({'description': 'Separate Buttons Menu', 'iconUrl': '/images/wysiwyg/skinIcons/menu/menubuttons.png', 'hidden': false, 'index': 7});
    ...
    def.css([
        ...
        '%.color% {[bg] [trans] [rd] [pos] [shd] border: [brw] solid [brd]; position: absolute;}',
        '%menuContainer% > li:hover > %.gap% %.color% {[bgh] [trans] border: [brw] solid [brdh];}',
        '%menuContainer% > li.selected > %.gap% %.color% {[bgs] border: [brw] solid [brds];}',
        '%menuContainer% > li.selected:hover > %.gap% %.color% {[bgs] border: [brw] solid [brds];}',

        '%.shade% {[rd] position: absolute; top: 0; bottom: 0; left: 0; right: 0;}',

        ' .subMenu {[rdDrop] [shd] border: [brw] solid [brd]; min-width: calc(100% - ([pad] * 2)); min-width: -webkit-calc(100% - ([pad] * 2)); z-index: 1; overflow: hidden;}',
        ' .subMenu li %.gap% {padding: 0; overflow: hidden; position: relative;}',
        ' .subMenu li %.bg% {padding: 5px 15px;}',
        ' .subMenu li %.color% {[bgDrop] box-shadow: none; border-radius: 0; border-width: 0;}',
        ...
        ' .subMenu li:hover %.shade% {border-radius: 0;}',
        ' .subMenu li.selected %.shade% {border-radius: 0;}',
