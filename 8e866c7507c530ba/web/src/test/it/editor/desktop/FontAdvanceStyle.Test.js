describe("FontAdvanceStyle Integration Tests", function () {
    ...
    function simulateFontFamilyChange(comp,locationInOptionLis){
        selectedFontBeforeChange=comp._fontFormatLogic._skinParts.comboBox._skinParts.select._skinParts.label.innerHTML;
        originalStyleData = addedComp._style.getProperty(fontSelectorField.propertyName);
        originalSource = addedComp._style.getPropertySource(fontSelectorField.propertyName);
        expect(comp).toBeDefined();
    ...
    function simulateDialogClose(dialog,action){
        var expectedInnerText = dialog._selectedFormat;
        var expectedStyleVal = dialog._selectedFont.value;
        if (action == "Cancel"){
            expectedInnerText=selectedFontBeforeChange;
            expectedStyleVal = originalStyleData;
            expectedStyleSource = originalSource;
        }
        dialog._dialogWindow.endDialog(action);
        expect(dialog.parentInstance._skinParts.font.innerHTML).toEqual(expectedInnerText);
        var currStyleDataFont = addedComp._style.getProperty(fontSelectorField.propertyName);
        var CurrPropertyFont =  addedComp._style.getPropertySource(fontSelectorField.propertyName);
        expect(currStyleDataFont).toEqual(expectedStyleVal);
        expect(CurrPropertyFont).toEqual(expectedStyleSource);
        advanceStylingDialog._dialogWindow._onButtonClicked("OK");
    }
      ...
      it("Should add a comp"+compToAdd.compData.comp +"and test several style changes", function () {
          automation.Utils.waitsForPromise(function() {
              return new Q()
                  ...
                  .then(function (comp){
                      addedComp=comp;
                      return openAdvanceStyleDialog(comp);
                  })
                  .then(waitForDialogToBeReady)
                  .then(function (dialog){
                      fontSelectorField = dialog;
                      return openFontAdvancedStyleDialog(dialog);
                  })
                  .then(waitForFontDialogToBeReady)
                  .then(function(comp){
                      return simulateFontFormatChange(comp,0);
                  })
                  .then(function(dialog){
                  ...
                  .then(waitForFontDialogToBeReady)
                  .then(function(comp){
                      return simulateFontFamilyChange(comp,103);
                  })
                  .then(function(dialog){
                      return simulateDialogClose(dialog,"Cancel");
                  })
                  .then(function (){
                      return openAdvanceStyleDialog(addedComp);
                  })
