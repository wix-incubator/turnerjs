...
class CameraScreen extends Component {
  ...
  constructor(props) {
    ...
    //this.currentRatioArrayPosition = -1;
    this.state = {
  ...
  renderCancelButton() {
    return (
      <TouchableOpacity
        style={{flex: 1, flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', padding: 10}}
    ...
    return (
      <View style={styles.captureButtonContainer}>
        <TouchableOpacity
          ...
          onPress={() => this.onCaptureImagePressed(this)}
        >
          <Image
  ...
  async onCaptureImagePressed() {
    if (this.state.canCaptureImage) {
      this.setState({canCaptureImage: false, captured: true});
      const image = await this.camera.capture(true);
      if (image && image.id) {
        await this.props.dispatch(imagesActions.imageCapture(image.id));
      }
      await this.setState({canCaptureImage: true});

      if (image && this.state.singleImageMode) {
