...
class Example extends Component {
  ...
  async onStartGroup(overlay, max) {
    ...
    try {
      ...
      console.warn('SUCCESS', 'back to caller');
    } catch (e) {
      this.setState({result: JSON.stringify(e)});
      console.warn('ERROR', 'back to caller');
    }
  }
  ...
  async onCheckPermission() {
    const hasPermission = await CameraKitCamera.hasCameraPermission();
    ToastAndroid.show('has Permission: ' + hasPermission, ToastAndroid.SHORT);
  }

  render() {
    return (
        <View style={{flex: 1, alignItems: 'center', justifyContent: 'center', marginTop: 100, backgroundColor: 'white'}}>
          ...
          <TouchableOpacity onPress={() => this.onCheckPermission()}>
            <Text style={styles.text}>
              Check permission
            </Text>
          </TouchableOpacity>
