define(['lodash', 'animations/localTweenEngine/localTweenEngine'], function(_, tweenEngine) {
    ...
    function animation(elements, duration, delay, params) {
        ...
        sequence.add(factory.animate('BaseFade', elements, duration * 0.75, delay + duration * 0.25, {from: {opacity: 1}, to: {opacity: 0}, ease: 'Cubic.easeInOut'}));
        ...
        _.forEach(elements, function(element) {
            ...
            var toClipParams = getClipTweenParams(element, adjDirection);

            sequence.add(factory.animate('BaseClip', element, duration, delay, {to: toClipParams, ease: 'Cubic.easeInOut'}), 0);
        });
    ...
    factory.registerAnimation('Conceal', animation);
});
