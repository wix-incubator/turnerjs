define(['lodash', 'animations/localTweenEngine/localTweenEngine'], function(_, tweenEngine) {
    ...
    function animation(elements, duration, delay, params) {
        ...
        sequence.add(factory.animate('BaseFade', elements, duration * 0.75, delay + (duration * 0.25), {from: {opacity: 1}, to: {opacity: 0}, ease: 'Sine.easeInOut'}));
        ...
        _.forEach(elements, function(element) {
            ...
            var toParams = {rotationX: paramsMap[adjDirection].angleX, rotationY: paramsMap[adjDirection].angleY};
            // the tween
            sequence.add([
                factory.animate('BasePosition', element, 0, delay, {from: {transformOrigin: originParams, x: transformXYParams.x, y: transformXYParams.y}}),
                factory.animate('BaseRotate3D', element, duration, delay, {to: toParams, perspective: 800, fallbackFor3D: true, ease: 'Cubic.easeInOut'})
            ], 0);
        });
    ...
    factory.registerAnimation('FoldOut', animation);
});
