    ...
    describe('tpaCompAPIMixin', function () {
        ...
        describe('Site events listening logic', function () {
            ...
            beforeEach(function() {
                ...
                var tpaPageNavigationAspect = this.siteAPI.getSiteAspect('tpaPageNavigationAspect');
                this.registerToPageChangedSpy = spyOn(tpaPageNavigationAspect, 'registerToPageChanged');

                var sessionChangedAspect = this.siteAPI.getSiteAspect('svSessionChangeEvent');
                this.registerToSessionChangedSpy = spyOn(sessionChangedAspect, 'registerToSessionChanged');
                this.unRegisterToSessionChangedSpy = spyOn(sessionChangedAspect, 'unRegisterToSessionChanged');
            ...
            it('should register to scroll event when startListen is called', function () {
                ...
                expect(this.registerToPageChangedSpy).toHaveBeenCalledWith(mixin, event);
            });
            ...
            it('should register to SESSION_CHANGED event when startListen is called', function () {
                var event = 'SESSION_CHANGED';
                mixin.startListen(event);

                expect(this.registerToSessionChangedSpy).toHaveBeenCalledWith(mixin);
                expect(mixin.isCompListensTo(event)).toBeTruthy();
            });

            it('should unregister to SESSION_CHANGED event when stopListen is called', function () {
                var event = 'SESSION_CHANGED';
                mixin.startListen(event);

                mixin.stopListen(event);

                expect(this.unRegisterToSessionChangedSpy ).toHaveBeenCalledWith(mixin);
                expect(mixin.isCompListensTo(event)).toBeFalsy();
            });
        });
