define(['lodash', 'jquery', 'bluebird'], function (_, $, Promise) {
    'use strict';
    ...
    PingPong.prototype.getContentWindow = function () {
        ...
        if (this.id === 'tpaSettings') {
            return document.getElementById('tpaSettingsFrame').contentWindow;
        }

        if (frames.preview && frames.preview.contentDocument) {
            previewDocument = frames.preview.contentDocument;
        }

        if (previewDocument) {
            var compWindow = previewDocument.getElementById(this.id + 'iframe');
            if (compWindow && compWindow.contentWindow) {
                return compWindow.contentWindow;
            }
        }
        return null;
    };
