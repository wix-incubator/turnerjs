...
export const ProductListItem = (props) => {
  return  <TouchableWithoutFeedback activeOpacity={props.isSearch ? 1 : 0.2} onPress={() => !props.isSearch && props.onPress()}>
      <View style={[
                        styles.listItem,
        ...
        {props.count ? (
          <View style={[{
            ...
            <Text style={{
                fontSize: 13,
                color: '#fff'
            }}>{props.count > 99 ? ':)' : props.count}</Text>
          </View>
        ) : null}
...
function getRest(props){
  ...
  return (
    <View style={[styles.labelsView, styles.labelItemView]}>
      ...
      {showPrice
        ? <Text numberOfLines={1} style={[styles.textTag, {textAlign: 'right', flex:0.5}]}>{discountPrice(props.productData, props.storeSettings)}</Text>
        : null
      }
    </View>
  );
